...
public class DriveMotionPlanner implements CSVWritable {
    //Max distance between each point (X, Y, Theata) once one maxes out, it drops a new point NOW IN METERS
    private static final double kMaxDx = 0.01;
    private static final double kMaxDy = 0.001;
    private static final double kMaxDTheta = Math.toRadians(2.5);
    ...
    //Follow type of the drive train
    ...
    //The model, a singleton
    final DifferentialDrive mModel;
    //An interator that iterates through a set of states, namely positions with curvatures that integrate together to make a spline
    TrajectoryIterator<TimedState<Pose2dWithCurvature>> mCurrentTrajectory;
    //Are you going forward?
    boolean mIsReversed = false;
    //Used for calculation of delta times
    double mLastTime = Double.POSITIVE_INFINITY;
    //This is a point of the current trajectory
    public TimedState<Pose2dWithCurvature> mSetpoint = new TimedState<>(Pose2dWithCurvature.identity());
    //This is the difference between where you are and where you should be
    Pose2d mError = Pose2d.identity();
    //The output
    Output mOutput = new Output();
    //The linear and angular velocity of the robot in the previous state
    DifferentialDrive.ChassisState prev_velocity_ = new DifferentialDrive.ChassisState();
    //Delta Time
    double mDt = 0.0;

    public DriveMotionPlanner() {
        //This is the motor model for the drive train
        final DCMotorTransmission transmission = new DCMotorTransmission(
                Constants.DRIVE_Kv,
                Constants.DRIVE_WHEEL_RADIUS * Constants.DRIVE_WHEEL_RADIUS * Constants.ROBOT_LINEAR_INERTIA / (2.0 * Constants.DRIVE_Ka),
                Constants.DRIVE_V_INTERCEPT);
        //Get EXCITED!!! This is the physical model of the robot
        mModel = new DifferentialDrive(
                Constants.ROBOT_LINEAR_INERTIA,
                Constants.ROBOT_ANGULAR_INERTIA,
                Constants.ROBOT_ANGULAR_DRAG,
                Constants.DRIVE_WHEEL_RADIUS,
                Constants.DRIVE_WHEEL_TRACK_WIDTH / 2.0 * Constants.TRACK_SCRUB_FACTOR,
    ...
    //Setting the spline to a given spline
    ...
    //This is where we make the path, and this is where the magic happens
    public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(
            boolean reversed,
            final List<Pose2d> waypoints,
            final List<TimingConstraint<Pose2dWithCurvature>> constraints,
    ...
    public int getNumFields(){
        return mSetpoint.getNumFields() + 4;
    }
    ...
    protected Output updatePID(DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
        ...
        adjusted_velocity.linear = dynamics.chassis_velocity.linear + kPathKX * mError.getTranslation().x();
        adjusted_velocity.angular = dynamics.chassis_velocity.angular + dynamics.chassis_velocity.linear * kPathKY *
                mError.getTranslation().y() + kPathKTheta * mError.getRotation().getRadians();
    ...
    protected Output updatePurePursuit(DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
        ...
        DifferentialDrive.ChassisState adjusted_velocity = new DifferentialDrive.ChassisState();
        // Feedback on longitudinal error (distance).
        adjusted_velocity.linear = dynamics.chassis_velocity.linear + Constants.Path_Kx * mError.getTranslation().x();
    ...
    protected Output updateNonlinearFeedback(DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
        ...
        final DifferentialDrive.ChassisState adjusted_velocity = new DifferentialDrive.ChassisState(
                ...
                dynamics.chassis_velocity.angular + k * angle_error_rads +
                        dynamics.chassis_velocity.linear * kBeta * sin_x_over_x * mError
                                .getTranslation().y());
    ...
    public Output update(double timestamp, Pose2d current_state) {
        ...
        if (!mCurrentTrajectory.isDone()) {
            ...
            final double curvature_m = mSetpoint.state().getCurvature();
            final double dcurvature_ds_m = mSetpoint.state()
                    .getDCurvatureDs();
